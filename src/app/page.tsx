...
export default function WordOfTheDayPage() {
    ...
    const [validationResult, setValidationResult] = useState<ValidateResponse>({
        score: 0,
        suggestion: "",
        feedback: "",
    });
    ...
    const computeRandomScore = (sentence: string) => {
        const words = sentence.trim().split(/\s+/).filter(Boolean).length;
        const lengthScore = Math.min(words, 20) / 20 * 10; // reward more complete sentences up to 20 words
        const variance = Math.random() * 2 - 1; // add a little randomness
        return Math.max(0, Math.min(10, parseFloat((lengthScore + variance).toFixed(1))));
    };
                ...
                {!isSubmitted ? (
                    ...
                    <div className="text-center">
                        ...
                        <div className="text-left bg-[#ECFDF5] border border-green-100 p-6 rounded-lg mb-8">
                            <p className="text-[#065F46] font-bold text-sm mb-2">Suggestion: {validationResult.suggestion}</p>
